		...
		init: function( editor ) {
			...
			editor.on( 'uiSpace', function( event ) {
				...
				for ( var r = 0; r < toolbar.length; r++ ) {
					...
					for ( var i = 0; i < items.length; i++ ) {
						...
						if ( item ) {
							...
							if ( !toolbarObj ) {
								...
								output.push( '<span id="', toolbarId, '" style="width:850px;'+(r==0?'':'display:none;')+'" class="cke_toolbar"', ( toolbarName ? ' aria-labelledby="' + toolbarId + '_label"' : '' ), ' role="toolbar">' );
					...
					if ( toolbarObj ){
						if(r==0){
							output.push( '<span class="cke_toolbar_end">' );
								...
								editor.addCommand( 'toolbarCollapse', {
									...
									exec: function( editor ) {
										var collapser = editor.ui.space( 'toolbar_collapser' ),
											toolbox = collapser.getParent().getParent().getParent(),
											...
											hideContent = toolbox.getChild(2);
